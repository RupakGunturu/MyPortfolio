...
const Hyperspeed = ({ effectOptions = {
  ...
  useEffect(() => {
    console.log("Hyperspeed mounted", hyperspeed.current);
    ...
    const xyUniforms = {
      ...
      uAmp: { value: new THREE.Vector2(60, 40) }
    };
    ...
    const turbulentUniforms = {
      uFreq: { value: new THREE.Vector4(10, 20, 20, 2) },
      uAmp: { value: new THREE.Vector4(60, 20, 30, 30) }
    };

    const deepUniforms = {
      uFreq: { value: new THREE.Vector2(8, 16) },
      uAmp: { value: new THREE.Vector2(40, 80) },
      uPowY: { value: new THREE.Vector2(40, 3) }
    };
    ...
    const distortions = {
      ...
      turbulentDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          let lookAtAmp = new THREE.Vector3(-6, -12, 0);
    ...
    if (hyperspeed.current && mounted) {
      effectOptions.distortion = distortions[effectOptions.distortion];
      myApp = new App(hyperspeed.current, effectOptions);
      myApp.loadAssets().then(myApp.init);
    }
    return () => {
      window.removeEventListener("mousedown", myApp?.onMouseDown);
      window.removeEventListener("mouseup", myApp?.onMouseUp);
      window.removeEventListener("mouseleave", myApp?.onMouseUp);
      window.removeEventListener("touchstart", myApp?.onMouseDown);
      window.removeEventListener("touchend", myApp?.onMouseUp);
      window.removeEventListener("touchcancel", myApp?.onMouseUp);
      mounted = false;
    };
  }, []);
